...
export const useRentalsStore = create<RentalsState>((set, get) => ({
    ...
    fetchRentals: async () => {
        ...
        try {
            ...
            const rentalsResponse: AxiosResponse<Rental[]> = await axios.get('http://localhost:3000/rentals', {
            ...
            const rentalsWithBooks = await Promise.all(
                rentalsResponse.data.map(async (rental) => {
                    if (rental.bookId) {
                        try {
                            const bookResponse: AxiosResponse<Book> = await axios.get(`http://localhost:3000/books/${rental.bookId}`);
            ...
            const errorMessage = axios.isAxiosError(error) ? handleApiError(error) : 'Wystąpił nieznany błąd.';
        ...
        try {            
            ...
            set((state) => ({
                rentals: state.rentals.map((r) =>
                    r.id === rentalId ? { ...r, status: 'Returned', returnDate: new Date().toISOString() } : r
                ),
                loading: false,
            }));
            ...
            if (rental && rental.bookId) {
                const updatedBooks = books.map(book =>
                    book.id === rental.bookId ? { ...book, availableCopies: (book.availableCopies ?? 0) + 1 } : book
                );
                setBooks(updatedBooks);
            }
        } catch (error) {
            console.error('Błąd podczas zwracania książki:', error);
            const errorMessage = axios.isAxiosError(error) ? handleApiError(error) : 'Wystąpił nieznany błąd podczas zwracania książki.';
    ...
    borrowBook: async (bookId: string | number) => {
        ...
        try {
            ...
            set((state) => ({ rentals: [...state.rentals, newRental], loading: false }));
            const { books, setBooks } = useBooksStore.getState();
            const updatedBooks = books.map(book =>
                book.id === bookId ? { ...book, availableCopies: (book.availableCopies ?? 0) - 1 } : book
            );
            setBooks(updatedBooks);            
        } catch (error) {
            console.error('Błąd podczas wypożyczania książki:', error); // Logowanie błędu
            const errorMessage = axios.isAxiosError(error) ? handleApiError(error) : 'Wystąpił nieznany błąd podczas wypożyczania książki.';
            set({ error: errorMessage, loading: false });
        }
    },
}));