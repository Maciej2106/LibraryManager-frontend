...
describe('useLogsStore', () => {
    ...
    it('should fetch logs correctly', async () => {
        const mockLogs: Log[] = [
            { user: 'user1', action: 'Login', timestamp: new Date().toISOString() },
            { user: 'user2', action: 'Logout', timestamp: new Date().toISOString() },
        ];
    ...
    it('should handle error during fetch logs', async () => {
        localStorage.setItem('token', 'test-token');
        mockAxios.onGet('http://localhost:3000/logs').reply(500, { message: 'Internal Server Error' });
    ...
    it('should handle missing token error', async () => {
        ...
        expect(useLogsStore.getState().loading).toBe(false);
        expect(useLogsStore.getState().error).toBe('Wystąpił nieoczekiwany błąd.');
    });
});